...
export async function postJson({ url, headers, body, signal, timeoutMs = DEFAULT_TIMEOUT_MS, }) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(new AiError('timeout', 'Request timed out.')), timeoutMs);
    const onAbort = () => controller.abort(new AiError('aborted', 'Request cancelled.'));
