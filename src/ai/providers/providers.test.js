...
function callAt(fetchMock, index) {
...
describe('OpenAiCompatibleProvider', () => {
    ...
    it('allows local providers without an API key', async () => {
        const local = new OpenAiCompatibleProvider(OPENAI_COMPATIBLE_PRESETS.find((p) => p.id === 'ollama'));
        const fetchMock = mockFetch({ choices: [{ message: { content: payload } }] });
    ...
    it('normalises transport failures', async () => {
        ...
        const error = await provider.explain(request, config).catch((e) => e);
        expect(error).toBeInstanceOf(AiError);
        expect(error.code).toBe('network');
    });
    it('propagates caller aborts', async () => {
        const controller = new AbortController();
        vi.stubGlobal('fetch', vi.fn((_u, init) => new Promise((_resolve, reject) => {
