 ...
 *
 * Note: bilingual reading does NOT use this limiter. Bilingual always routes
 * through keyless Google Translate (independent of the user's AI key), so it
 * has its own path. This limiter is shared only by explanation-style
 * capabilities (auto-explain, enrichment).
 */
 ...
 * enrichment) so concurrent requests there do not burst a provider. Defaults to
 * at most 5 requests per 10 seconds — friendly to local models and free tiers
 * alike. Bilingual reading does NOT use this (it is keyless Google, see
 * translate-service).
 */
