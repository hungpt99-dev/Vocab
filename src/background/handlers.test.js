...
import { buildHighlightData, createHandlers, explainWord, readActiveSelection, saveSelection, } from './handlers';
...
describe('saveSelection', () => {
    ...
    it('still saves when auto-explain fails', async () => {
        await deps.settings.update({ autoExplainOnSave: true });
        deps.explain.explainWith.mockRejectedValue(new Error('no key'));
        const entry = await saveSelection(deps, selection);
...
describe('createHandlers', () => {
    ...
    it('handles explain', async () => {
        const result = await dispatch(createHandlers(deps), { type: 'explain', payload: { word: 'cake' } }, sender);
