...
export class VocabularyRepository {
    ...
    async update(id, patch) {
        ...
        if (patch.sentence !== undefined)
            next.sentence = collapseWhitespace(patch.sentence);
    ...
    async list(query = {}) {
        const { search = '', favoritesOnly = false, tag = '', sortBy = 'createdAt', sortDirection = 'desc', limit, offset = 0, } = query;
        ...
        entries.sort((a, b) => {
            const result = sortBy === 'word' ? a.wordKey.localeCompare(b.wordKey) : a.createdAt - b.createdAt;
     ...
     */
    async importAll(entries, mode = 'merge') {
        if (mode === 'replace') {
            await this.db.vocabulary.clear();
        }
        let imported = 0;
...
function matchesTerm(entry, term) {
    return (entry.wordKey.includes(term) ||
