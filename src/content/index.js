...
function scan(root) {
    if (!root)
        return;
    const run = () => void highlightRoot(root, matcher);
...
function startObserving() {
    ...
    observer = new MutationObserver((mutations) => {
        const touchedContent = mutations.some((mutation) => [...mutation.addedNodes].some((node) => !isOwnNode(node)));
        if (!touchedContent)
            return;
