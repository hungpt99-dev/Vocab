...
| L10 | **Vocabulary Radar MVP scope** | One goal at a time (the `radar.goal` string in Settings); manual scan ("Find for my Radar") plus optional auto-find on page load (when enabled, or when Bilingual mode is on); AI-extracted `domains` are an optional per-domain allow-list, not required; in-flight cancellation across the message channel is not wired (the service accepts an `AbortSignal` for future/tests) | Multi-goal management UI, live cancel message, optional goal-metadata enrichment as fast-follows |
